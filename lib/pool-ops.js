...
export async function mergePools(inputs, outputDir, options = {}) {
  ...
  const enhancementStats = await enhanceOutputPapers(merged.papers, {
    unpaywall: engines.unpaywall !== false ? new UnpaywallAPI(apiKeys.unpaywallEmail || apiKeys.crossrefMailto, options.logger || console) : null,
    openCitations: engines.openCitations === true ? new OpenCitationsAPI(options.logger || console) : null
  });
...
export async function enrichMetadata(inputPath, options = {}) {
  ...
  const result = await enrichMetadataInPool(pool, {
    ...options,
    onCheckpoint: checkpointInterval > 0
      ? checkpoint => {
        writeResultFiles(checkpoint.pool, outputDir, {
          mode: 'enrich-checkpoint',
          outputDir
        });
        options.logger?.info?.(`   Checkpoint saved after ${checkpoint.stats.complete + checkpoint.stats.attempted}/${checkpoint.stats.total} paper(s)`);
      }
...
function mergePaperMetadata(target, source) {
  ...
  target.citation_metadata = chooseCitationMetadata(target.citation_metadata, source.citation_metadata);
  target.publication_status = choosePublicationStatus(target.publication_status, source.publication_status);
