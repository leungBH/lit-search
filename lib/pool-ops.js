...
export async function resolveCitationsFile(filePath, options) {
  ...
  for (let i = 0; i < citationTotal; i++) {
    throwIfCancelled(signal, 'Citation resolve cancelled by client');
    const citation = citations[i];
    const query = citation.doi || citation.title;
    if (!query) {
      unresolved.push(citation);
      if (onProgress)
        await onProgress(i + 1, citationTotal, `跳过空引用 ${i + 1}/${citationTotal}`);
      continue;
    }
    ...
    if (onProgress) {
      await onProgress(i + 1, citationTotal, `解析引用 ${i + 1}/${citationTotal}`);
    }
  }
