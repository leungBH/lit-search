...
const COMPLETENESS_FIELDS = [
  'abstract', 'authors', 'venue', 'year', 'doi',
  'citationCount', 'volume', 'issue', 'pages'
];
...
export async function lookupPaper({
  ...
  const tasks = sourceList.map(source => safeLookup(source, target, inputType, apiKeys, logger, signal));
...
function mergeHits(hits, inputType, target) {
  const ranked = [...hits].sort(
    (a, b) => paperCompleteness(b.paper) - paperCompleteness(a.paper)
  );
  ...
  merged.identifiers = mergeIdentifiers(hits.map(h => p => h.paper.identifiers || {}));
  merged._lookup = {
    inputType,
    target,
    sources: hits.map(h => h.source),
