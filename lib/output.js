import { getBestPdfCandidateUrl, getDownloadablePdfCandidates } from './pdf-candidates.js';
...
export function renderMarkdown(result) {
  ...
  for (const paper of papers) {
    ...
    lines.push(`- 摘要状态: ${formatAbstractStatus(paper)}`);
    ...
    lines.push(`- PDF候选: ${formatPdfCandidates(paper.pdf_candidates)}`);
...
export function renderPdfStatus(result) {
  ...
  lines.push('');
  lines.push('| # | Title | DOI | PDF Candidates | Status | Failure reason | Local path | Next action |');
  lines.push('| --- | --- | --- | --- | --- | --- | --- | --- |');
  ...
  for (const paper of papers) {
    ...
    lines.push([
      ...
      escapeTable(formatPdfCandidateSummary(paper.pdf_candidates)),
...
function renderBibEntry(paper) {
  ...
  const bestPdfCandidateUrl = getBestPdfCandidateUrl(paper);
  if (bestPdfCandidateUrl) {
    fields.push(['pdfurl', bestPdfCandidateUrl]);
  }

  if (paper.pdf_candidates?.length) {
    fields.push(['pdfcandidates', paper.pdf_candidates.map(candidate => `${candidate.rank}:${candidate.access_type}:${candidate.url}`).join('; ')]);
  }
...
function formatDownloadNote(paper) {
  ...
  if (!getDownloadablePdfCandidates(paper).length) {
    return '无可直接下载的 PDF 候选；可查看 DOI/落地页候选';
  }
...
function formatAbstractStatus(paper) {
  const status = paper.abstract_status || (hasText(paper.abstract) ? 'present' : 'missing');
  const source = paper.abstract_source || 'N/A';
...
function getPdfStatus(paper) {
  ...
  if (!paper.pdf_candidates?.length) {
    return {
      status: 'missing_url',
      failureReason: 'No PDF candidate URL was provided by this source.',
...
function formatPdfCandidates(candidates = []) {
  if (!candidates.length) return 'N/A';
  return candidates
    .slice(0, 3)
    .map(candidate => `#${candidate.rank} ${candidate.url} (${candidate.access_type}, confidence ${candidate.confidence})`)
    .join('; ');
}

function formatPdfCandidateSummary(candidates = []) {
  if (!candidates.length) return 'N/A';
  return candidates
    .map(candidate => `#${candidate.rank} ${candidate.access_type} ${candidate.url}`)
    .join('; ');
}
