...
export function buildPdfCandidate(input = {}) {
  ...
  const confidence = normalizeConfidence(
    input.confidence ?? buildConfidence(baseScore, input, url)
  );
...
export function getDownloadablePdfCandidates(paper) {
  return normalizePdfCandidates(paper.pdf_candidates || []).filter((candidate) =>
    DOWNLOADABLE_ACCESS_TYPES.has(candidate.access_type)
  );
}
...
function inferAccessType(url, input = {}) {
  ...
  if (sourceType === 'repository') {
    return /(university|institution|repository|archive|repo|scholarworks|eprints|dspace|ir\.)/i.test(
      `${host} ${providerText}`
    )
      ? 'institutional_repository'
      : 'repository';
...
function buildReason(accessType, provider) {
  const reasons = {
    ...
    institutional_repository:
      'Institutional repository candidates are preferred over publisher landing pages.',
