...
function buildInitialPdfSummary(papers, outputDir) {
  return {
    ...
    results: papers.map(paper => ({
      paper,
      status: getDownloadablePdfCandidates(paper).length ? 'not_attempted' : 'skipped',
      code: getDownloadablePdfCandidates(paper).length ? 'not_attempted' : 'no_pdf_candidates',
      message: getDownloadablePdfCandidates(paper).length ? 'PDF download has not been attempted.' : 'No downloadable PDF candidate was provided by this source.',
      action: getDownloadablePdfCandidates(paper).length ? 'Run lit-search pdf on this literature pool to download available PDF candidates.' : 'Use DOI/title to search open-access repositories or publisher landing pages.'
    }))
  };
}
...
export function toStructuredPdfSummary(pdfSummary) {
  return {
    ...
    results: pdfSummary.results.map(item => ({
      ...
      pdf_candidates: item.paper?.pdf_candidates || [],
      status: item.status,
      code: item.code || null,
