...
export async function resolvePublicationsInPool(pool, options = {}) {
  ...
  const openalex =
    options.openalex ||
    new OpenAlexAPI(options.apiKeys?.openalex || null, options.logger || console);
  const stats = {
    enabled,
    resolvePreprint,
    preferPublished,
  ...
  const papers = [];
  const total = pool.papers?.length || 0;
  for (let i = 0; i < total; i++) {
    // 每篇前检查取消
    throwIfCancelled(signal, 'Publication resolution cancelled by client');
    const paper = pool.papers[i];
    const resolved = enabled
      ? await resolvePublicationForPaper(paper, {
          openalex,
          preferPublished,
    ...
    if (onProgress) {
      await onProgress(i + 1, total, `解析出版信息 ${i + 1}/${total}`);
    }
  }
...
export async function resolvePublicationForPaper(paper, options = {}) {
  ...
  const resolved = {
    ...
    citation_metadata_preference: options.preferPublished
      ? PUBLISHED_PREFERENCE
      : BEST_AVAILABLE_PREFERENCE,
    metadata_sources: {
...
export function attachPublicationModel(paper) {
  ...
  const existingCitation =
    paper.citation_metadata || buildCitationMetadata(paper, paper.source || 'original', 0.5);
  const publicationStatus =
    paper.publication_status || inferPublicationStatus({ ...paper, identity, preprint });
  ...
  return {
    ...
    citation_metadata_preference:
      paper.citation_metadata_preference ||
...
async function findFormalRecord(paper, options) {
  ...
  const openalexId =
    paper.identity?.openalex_id || paper.openalex_id || paper.identifiers?.openalex;
  ...
  const matches = await safeLookup(() =>
    openalex.searchWorks(paper.title, {
      limit: 3,
      searchScope: 'title-only',
      yearRange: paper.year
        ? { start: Math.max(1900, Number(paper.year) - 1), end: Number(paper.year) + 2 }
        : null,
    })
  );
  return (
    (Array.isArray(matches) ? matches : []).find((match) =>
      isHighConfidenceFormalMatch(paper, match)
    ) || null
  );
}
...
function isHighConfidenceFormalMatch(original, candidate) {
  ...
  const yearOk =
    !original.year ||
    !candidate.year ||
    Math.abs(Number(original.year) - Number(candidate.year)) <= 2;
...
function isArxivLike(paper) {
  const text = [paper.source, paper.journal, paper.venue, paper.publisher, paper.url]
    .filter(Boolean)
    .join(' ');
...
function buildFingerprint(paper) {
  const firstAuthor =
    Array.isArray(paper.authors) && paper.authors.length > 0 ? paper.authors[0] : '';
  return (
    [normalizeTitle(paper.title), normalizePerson(firstAuthor), paper.year || '']
      .filter(Boolean)
      .join('|') || null
  );
}
...
function hasSharedAuthor(a = [], b = []) {
  const left = new Set(
    (Array.isArray(a) ? a : []).map(normalizePerson).filter(Boolean).map(lastName)
  );
