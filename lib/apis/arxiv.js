...
export class ArxivAPI {
  ...
  async fetchById(arxivId, signal = null) {
    ...
    try {
      const response = await fetchArxiv(`${BASE_URL}?${params}`, request.signal, this.queryTimeoutMs);
      if (!response.ok) return null;
      const entry = this._parseXML(response.body)[0];
      return entry ? this._normalizeEntry(entry) : null;
    } catch {
      return null;
    } finally {
      request.cleanup();
  ...
  /**
   * Look up a single arXiv paper by exact title. Returns the best match or null.
   * Throws on network errors.
   */
  async lookupByTitle(title, { signal } = {}) {
    if (!title) return null;
    const escaped = title.replace(/["()]/g, ' ').trim();
    const searchQuery = `ti:"${escaped}"`;
    const params = new URLSearchParams({
      search_query: searchQuery,
      max_results: '1'
    });
    ...
    try {
      ...
      if (response.status === 404) {
        const e = new Error('arXiv: not found');
        e.status = 404;
        e.source = 'arxiv';
        throw e;
      }
      if (!response.ok) {
        const e = new Error(`arXiv lookup error: ${response.status}`);
