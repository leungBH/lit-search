...
export class DoajAPI {
  ...
  _normalizeItem(item) {
    ...
    return {
      ...
      topics: [],
      language: Array.isArray(bibjson.language) ? bibjson.language.join(', ') : bibjson.language || null,
...
function collectDoajPdfCandidates(bibjson, doi) {
  ...
  for (const link of bibjson.link || []) {
    ...
    const looksPdf = type.includes('fulltext') || type.includes('pdf') || /\.pdf($|[?#])/i.test(link.url);
