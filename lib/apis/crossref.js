...
export class CrossrefAPI {
  ...
  async lookupByDoi(doi, { signal } = {}) {
    ...
    const request = createRequestSignal(signal, 15000);
    try {
      const response = await fetch(`${BASE_URL}/${encodeURIComponent(stripDoiPrefix(doi))}`, {
        headers,
        signal: request.signal,
      });
...
// Strip common DOI prefixes (URL, doi: scheme) before encoding into the path.
// Kept local to avoid a circular import from ../paper-lookup.js.
function stripDoiPrefix(raw) {
  return String(raw || '')
    .trim()
