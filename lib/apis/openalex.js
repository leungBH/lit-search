...
export class OpenAlexAPI {
  ...
  async searchWorks(query, options = {}) {
    ...
    while (papers.length < limit) {
      ...
      const params = new URLSearchParams({
        ...
        select:
          'id,title,display_name,authorships,publication_year,publication_date,primary_location,best_oa_location,locations,doi,open_access,cited_by_count,abstract_inverted_index,biblio,keywords,topics,language,type',
      });
  ...
  async fetchWorkByDoi(doi, signal) {
    if (!doi) return null;
    return this._fetchWorkByIdentifier(`doi:${normalizeDoi(doi)}`, signal);
  }

  async fetchWorkById(workId, signal) {
    if (!workId) return null;
    return this._fetchWorkByIdentifier(workId, signal);
  }

  async _fetchWorkByIdentifier(workId, signal) {
    const headers = {};
    if (this.apiKey) {
      headers['Authorization'] = `Bearer ${this.apiKey}`;
    }
    ...
    const url = new URL(
      workId.startsWith('http') ? workId : `${BASE_URL}/works/${encodeURIComponent(workId)}`
    );
    if (!this.apiKey) {
      url.searchParams.set('mailto', 'lit-search@example.com');
    }
    ...
    try {
      ...
      return this._normalizeWork(await response.json());
    } catch {
      return null;
    } finally {
      request.cleanup();
   ...
   * Throws on network/abort errors so the orchestrator can decide fallback.
   */
  async lookupByDoi(doi, { signal } = {}) {
    if (!doi) return null;
    const headers = {};
    if (this.apiKey) headers['Authorization'] = `Bearer ${this.apiKey}`;
    const url = new URL(`${BASE_URL}/works/doi:${encodeURIComponent(normalizeDoi(doi))}`);
    if (!this.apiKey) url.searchParams.set('mailto', 'lit-search@example.com');
    const request = createRequestSignal(signal, 15000);
    try {
      const response = await fetch(url, { headers, signal: request.signal });
      if (response.status === 404) {
        const e = new Error('OpenAlex: not found');
        e.status = 404;
        e.source = 'openalex';
        throw e;
      }
      if (!response.ok) {
        const e = new Error(`OpenAlex lookup error: ${response.status}`);
  ...
  async lookupByTitle(title, { signal } = {}) {
    ...
    try {
      ...
      return first ? this._normalizeWork(first) : null;
    } finally {
      request.cleanup();
    }
  }
  ...
  const addLocation = (location, role) => {
    ...
    if (pdfUrl) {
      candidates.push({
        source: 'openalex',
        provider:
          location.source?.display_name || location.source?.host_organization_name || 'openalex',
        url: pdfUrl,
        sourceType: location.source?.type || null,
        sourceName:
          location.source?.display_name || location.source?.host_organization_name || null,
        access_type: inferOpenAlexAccessType(location, role),
        license: location.license || null,
        is_oa: location.is_oa === true,
        confidence:
          scoreOpenAlexPdfCandidate({
            url: pdfUrl,
            role,
            sourceType: location.source?.type || null,
            sourceName:
...
function inferOpenAlexAccessType(location, role) {
  ...
  const sourceName = String(
    location?.source?.display_name || location?.source?.host_organization_name || ''
  ).toLowerCase();
  ...
  if (sourceType === 'repository') {
    return /(university|institution|repository|archive|scholarworks|eprints|dspace|ir\.)/i.test(
      `${host} ${sourceName}`
    )
...
function scoreOpenAlexPdfCandidate(candidate) {
  ...
  if (
    /(arxiv|pubmedcentral|ncbi|pmc|core\.ac\.uk|zenodo|figshare|osf|hal\.science|biorxiv|medrxiv|ssrn)/i.test(
      host
    )
  ) {
    score += 25;
  ...
  if (
    /(repository|archive|institutional|university|preprint|pubmed central|arxiv|core)/i.test(
      sourceName
    )
  ) {
    score += 15;
  ...
  if (
    /(elsevier|sciencedirect|springer|wiley|tandfonline|ieee|acm|sagepub|oup|cambridge|nature)/i.test(
      host
    )
  ) {
    score -= 20;
...
function shouldEnrichOpenAlexPaper(paper) {
  return (
    !paper.abstract || !paper.keywords?.length || (!paper.volume && !paper.pages) || !paper.journal
  );
}
