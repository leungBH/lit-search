...
function buildPubMedQuery(query, searchScope, yearRange) {
  ...
  if (yearRange?.start || yearRange?.end) {
    parts.push(
      `("${yearRange.start || 1800}"[Date - Publication] : "${yearRange.end || 3000}"[Date - Publication])`
    );
  }
...
function parsePubMedArticles(xml) {
  ...
  return articles.map((article) => {
    ...
    const journal =
      stripTags(extractTag(article, 'Title')) || extractTag(article, 'ISOAbbreviation') || null;
...
function parseAbstract(article) {
  const values = Array.from(
    article.matchAll(/<AbstractText[^>]*>([\s\S]*?)<\/AbstractText>/gi),
...
function parseKeywords(article) {
  return Array.from(article.matchAll(/<Keyword[^>]*>([\s\S]*?)<\/Keyword>/gi), (match) =>
    stripTags(match[1])
  ).filter(Boolean);
}
...
function parseMesh(article) {
  return Array.from(
    article.matchAll(/<DescriptorName[^>]*>([\s\S]*?)<\/DescriptorName>/gi),
    (match) => stripTags(match[1])
  ).filter(Boolean);
}
...
function extractArticleId(article, type) {
  const match = article.match(
    new RegExp(`<ArticleId[^>]*IdType="${type}"[^>]*>([\\s\\S]*?)</ArticleId>`, 'i')
  );
