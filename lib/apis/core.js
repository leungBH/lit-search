...
export class CoreAPI {
  ...
  _normalizeWork(work) {
    const authors = (work.authors || [])
      .map(a => a.name || '')
      .filter(Boolean);
    const pdfCandidates = collectCorePdfCandidates(work);
...
function collectCorePdfCandidates(work) {
  const candidates = [];
  ...
  if (work.downloadUrl) {
    candidates.push({
      ...
      is_oa: true,
      confidence: 0.86,
      reason: 'CORE downloadUrl was provided for this work.',
      resolver: 'core.downloadUrl'
    });
  }

  for (const outputUrl of work.outputs || []) {
    candidates.push({
      source: 'core',
      provider: safeHostname(outputUrl) || 'core',
      url: outputUrl,
      access_type: /\.pdf($|[?#])/i.test(outputUrl) ? 'repository' : 'browser_fallback',
      license: work.license || null,
      is_oa: true,
      confidence: /\.pdf($|[?#])/i.test(outputUrl) ? 0.7 : 0.2,
      reason: 'CORE output URL was provided for this work.',
      resolver: 'core.outputs'
    });
  }
