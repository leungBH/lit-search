...
const SEARCH_FIELD_SETS = [
  ...
  ['paperId', 'title', 'year', 'authors', 'venue', 'externalIds', 'url'].join(','),
];
...
export class SemanticScholarAPI {
        ...
        try {
          ...
          if (!response.ok) {
            if (response.status >= 500 && fieldSetIndex < SEARCH_FIELD_SETS.length - 1) {
              ...
              retryCount = 0;
              this.logger.info(
                `    ↪ Semantic Scholar 返回 ${response.status}，尝试精简字段重试...`
              );
            ...
            if (response.status === 429 || response.status >= 500) {
              ...
              const retryDelay = response.status === 429 ? this._handleRateLimit(response) : 2000;
  ...
  async _enrichPapers(papers, signal) {
    ...
    try {
      const response = await fetch(
        `${BASE_URL}/paper/batch?fields=${encodeURIComponent(DETAIL_FIELDS)}`,
        {
          method: 'POST',
          headers,
      ...
      if (!response.ok) {
        if (response.status === 429) {
          ...
          this.logger.info(
            `    ↪ Semantic Scholar 详情接口限流，跳过详情补全；建议 ${Math.ceil(retryDelay / 1000)} 秒后重试`
          );
        }
        if (response.status >= 500) {
          this.logger.info(
            `    ↪ Semantic Scholar 详情接口暂时不可用（HTTP ${response.status}），跳过详情补全`
          );
        }
        return papers;
      }
  ...
  async fetchPaper(identifier, signal) {
    ...
    try {
      const response = await fetch(
        `${BASE_URL}/paper/${encodeURIComponent(identifier)}?fields=${encodeURIComponent(DETAIL_FIELDS)}`,
        {
          headers,
          signal: request.signal,
        }
  ...
  _handleRateLimit(response) {
    const retryAfterMs =
      parseRetryAfterMs(response.headers?.get?.('retry-after')) || DEFAULT_RATE_LIMIT_DELAY_MS;
    this.paginationDelayMs = Math.max(this.paginationDelayMs, retryAfterMs);
    this.logger.info(
      `    ↪ Semantic Scholar 触发限流（HTTP 429），按 Retry-After 等待 ${Math.ceil(retryAfterMs / 1000)} 秒并临时放慢后续请求`
    );
    return retryAfterMs;
  }
}
...
function preferArray(primary, fallback) {
  return Array.isArray(primary) && primary.length
    ? primary
