...
import { getDownloadablePdfCandidates } from './pdf-candidates.js';
...
export async function downloadPaperPdfs(papers, outputDir, options = {}) {
  ...
  for (const paper of papers) {
    const candidates = getDownloadablePdfCandidates(paper);
    if (!candidates.length) {
      const failure = buildPdfFailure({
        status: 'skipped',
        code: 'no_pdf_candidates',
        message: 'No downloadable PDF candidate was provided by this source.',
    ...
    if (existsSync(filePath)) {
      ...
      logger.info?.(`   PDF ${paper.seq_id}: already exists ${fileName}`);
      continue;
    }
    ...
    for (const candidate of candidates) {
      try {
        const result = await downloadPdf(candidate.url, filePath, options);
        const success = {
          status: 'success',
          code: 'downloaded',
        ...
        const failure = normalizeDownloadError(error, candidate.url);
        attempts.push({
          candidate,
          status: 'failed',
          code: failure.code,
    ...
    if (!downloaded) {
      const lastFailure = attempts.at(-1) || buildPdfFailure({
        code: 'download_failed',
        message: 'All PDF candidates failed.',
        action: 'Try another source, DOI lookup, or manual browser access.'
      });
      const failure = buildPdfFailure({
        code: lastFailure.code,
        message: `All PDF candidates failed. Last error: ${lastFailure.message}`,
        action: lastFailure.action,
        url: lastFailure.candidate?.url || null,
...
function buildPdfFileName(paper) {
  ...
  const firstCandidateUrl = getDownloadablePdfCandidates(paper)[0]?.url;
  const title = sanitizeFilePart(paper.title || basename(String(firstCandidateUrl || 'paper')));
