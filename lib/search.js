...
export async function searchPapers(options) {
  ...
  const unpaywallApi = new UnpaywallAPI(apiKeys.unpaywallEmail || apiKeys.crossrefMailto, activeLogger);
  ...
  if (enabledEngines.europePmc) engineList.push({ name: 'Europe PMC', api: europePmcApi, method: 'searchWorks' });
  ...
  if (enabledEngines.pubmed) engineList.push({ name: 'PubMed', api: pubmedApi, method: 'searchWorks' });

  activeLogger.info(`🚦 数据源并发: ${engineList.length} 个源同时检索；同一源内关键词串行请求，间隔 ${(SAME_SOURCE_QUERY_DELAY_MS / 1000).toFixed(1)} 秒以降低限流风险\n`);
  ...
  for (let queryIndex = 0; queryIndex < queries.length; queryIndex++) {
    ...
    const queryOutputs = await Promise.all(
      engineList.map((engine, index) => searchEngineQuery({
        engine,
        index,
        query: q,
        limit,
  ...
  const engineOutputs = engineStates.map(state => finalizeEngineState({
    state,
    yearRange,
    logger: activeLogger
  }));
  ...
  let filtered = relevanceFilter ? filterByRelevance(deduped, strictFilterKeywords, excludeTerms) : deduped;
  ...
  if (relevanceFilter && filtered.length === 0 && deduped.length > 0) {
    ...
    if (relaxedFiltered.length > 0) {
      ...
      activeLogger.info(`💡 严格过滤无结果，已放宽为单词级关键词过滤: ${relaxedFilterKeywords.join(', ')}`);
    }
  }
  ...
  let outputPapers = sorted.map((p, idx) => toOutputPaper(p, idx + 1));

  const enhancementStats = await enhanceOutputPapers(outputPapers, {
...
async function searchEngineQuery({ engine, index, query, limit, yearRange, searchScope }) {
...
export async function enhanceOutputPapers(papers, enhancers) {
  const stats = {
  ...
  for (const paper of papers) {
    ...
    if (enhancers.unpaywall?.email) {
      ...
      const result = await enhancers.unpaywall.fetchByDoi(doi);
      if (result) {
        ...
        paper.pdf_candidates = mergePdfCandidates(paper.pdf_candidates || [], result.pdfCandidates || []);
    ...
    if (enhancers.openCitations) {
      ...
      const relations = await enhancers.openCitations.fetchCitationRelations(doi);
...
function deduplicate(papers) {
  ...
  for (const paper of papers) {
    if (paper.doi) {
      ...
      const existing = result.find(item => item.doi && item.doi.toLowerCase().trim() === normalizedDoi);
      if (existing) {
    ...
    for (const existingTitle of titleSet) {
      if (similarity(normalizedTitle, existingTitle) > 0.85) {
        duplicatePaper = result.find(item => item.title.toLowerCase().trim() === existingTitle) || null;
        break;
      }
    }
...
function buildRelaxedFilterKeywords(keywords) {
  const stopwords = new Set([
    ...
    'of', 'on', 'or', 'the', 'to', 'with', 'using', 'based', 'via'
  ]);
