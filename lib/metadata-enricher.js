...
export async function enrichMetadataInPool(pool, options = {}) {
  ...
  updatePoolMetadata(pool, { overwrite, onlyMissing, fields, concurrency, checkpointInterval, stats });
  ...
  async function processPaper(paper) {
    ...
    logger?.info?.(`   Metadata ${paper.seq_id || ''}: ${result.enrichedFields.length ? `enriched ${result.enrichedFields.length} field(s)` : 'no updates'}`);
    await maybeCheckpoint();
  }
  ...
  async function maybeCheckpoint() {
    ...
    updatePoolMetadata(pool, { overwrite, onlyMissing, fields, concurrency, checkpointInterval, stats });
    checkpointChain = checkpointChain.then(() => onCheckpoint({ pool, stats, reason: 'checkpoint' }));
    await checkpointChain;
  }
}
...
function createDefaultResolvers(options) {
  ...
  return {
    ...
    titleSearch: async (title, paper, opts) => {
      ...
      return [...openalexResults, ...semanticResults]
        .filter(candidate => titleSimilarity(normalize(candidate.title), normalize(paper.title)) > 0.9)
...
function mergeMetadataFromCandidate(paper, candidate, options) {
  ...
  setScalarField(paper, candidate, 'abstract', fields, overwrite, resolverLabel, resolvedFields, changed);
  setScalarField(paper, candidate, 'journal', fields, overwrite, resolverLabel, resolvedFields, changed);
  setScalarField(paper, candidate, 'venue', fields, overwrite, resolverLabel, resolvedFields, changed);
  setScalarField(paper, candidate, 'doi', fields, overwrite, resolverLabel, resolvedFields, changed);
  setScalarField(paper, candidate, 'url', fields, overwrite, resolverLabel, resolvedFields, changed);
  setScalarField(paper, candidate, 'volume', fields, overwrite, resolverLabel, resolvedFields, changed);
  setScalarField(paper, candidate, 'issue', fields, overwrite, resolverLabel, resolvedFields, changed);
  setScalarField(paper, candidate, 'pages', fields, overwrite, resolverLabel, resolvedFields, changed);
  setScalarField(paper, candidate, 'publisher', fields, overwrite, resolverLabel, resolvedFields, changed);
  setScalarField(paper, candidate, 'language', fields, overwrite, resolverLabel, resolvedFields, changed);
  setScalarField(paper, candidate, 'work_type', fields, overwrite, resolverLabel, resolvedFields, changed, 'workType');

  if (fields.includes('keywords')) {
    const merged = mergeList(paper.keywords, candidate.keywords, candidate.topics, candidate.fieldsOfStudy);
  ...
  if (fields.includes('identifiers') && candidate.identifiers && Object.keys(candidate.identifiers).length) {
  ...
  if (fields.includes('pdf_candidates')) {
    ...
    paper.pdf_candidates = mergePdfCandidates(paper.pdf_candidates || [], candidate.pdfCandidates || [], candidate.pdf_candidates || []);
...
function setScalarField(paper, candidate, field, fields, overwrite, resolverLabel, resolvedFields, changed, candidateField = field) {
...
function hasFieldValue(paper, field) {
  ...
  if (field === 'identifiers') return paper.identifiers && Object.values(paper.identifiers).some(Boolean);
  if (field === 'pdf_candidates') return Array.isArray(paper.pdf_candidates) && paper.pdf_candidates.length > 0;
