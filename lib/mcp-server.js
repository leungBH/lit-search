/**
 * lit-search MCP server factory.
 *
 * Centralizes all tool registration + helpers so both the stdio entry point
 * (`bin/lit-search-mcp.js`) and the in-process test transport can share the
 * exact same server setup.
 */
...
import { z } from 'zod';
import { runLitSearchWorkflow } from './workflow.js';
import { enrichMetadata, mergePools, resolveCitationsFile } from './pool-ops.js';
import { getResolvedApiKeys } from './app-config.js';
import { silentLogger } from './logger.js';
import { lookupPaper } from './paper-lookup.js';
import { wrapError } from './errors.js';
import { createProgressReporter } from './progress.js';

/**
 * Build a fully-configured lit-search McpServer.
 *
 * @param {object} options
 * @param {object} options.config - lit-search app config (createAppConfig()).
 * @param {string} [options.version] - server version, defaults to package.json.
 * @param {string|null} [options.debugLogFile] - optional debug log file path.
 * @returns {McpServer}
 */
export function createLitSearchMcpServer({ config, version = 'unknown', debugLogFile = null }) {
  const logDebug = makeDebugLogger(debugLogFile);
  ...
  server.registerTool(
    'search_literature',
    {
      ...
      inputSchema: {
        ...
        limit: z
          .number()
          .optional()
          .describe('Per-keyword, per-source retrieval limit. Default: 3.'),
    ...
    async (args, extra) => {
      ...
      const workflow = await runLitSearchWorkflow({
        ...
        engines: buildRuntimeEngines(args, config),
        logger: silentLogger,
  ...
  server.registerTool(
    ...
    async (args, extra) => {
      ...
      const progress = createProgressReporter(extra);
      const outputDir = args.outputDir || 'merged_literature';
  ...
  server.registerTool(
    ...
    async (args, extra) => {
      ...
      const progress = createProgressReporter(extra);
      const outputDir = args.outputDir || 'resolved_literature';
  ...
  server.registerTool(
    ...
    async (args) => {
      ...
      try {
        ...
        return mcpErrorResponse(wrapError(err), logDebug);
      }
    }
  );
...
function buildRuntimeEngines(args = {}, config) {
  return {
    ...(config.get('engines') || {}),
...
function mcpErrorResponse(err, logDebug = () => {}) {
...
function makeDebugLogger(debugLogFile) {
  return function logDebug(message) {
